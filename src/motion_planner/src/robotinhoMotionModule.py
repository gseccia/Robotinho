...
class Pose():
    ...
    def __add__(self, point):
        return Pose(self.x + point.x, self.y + point.y, self.theta + point.theta, self.linear_vel + point.linear_vel,
                    self.radial_vel + point.radial_vel)
    ...
    def __mul__(self, value):
        return Pose(self.x * value, self.y * value, self.theta * value, self.linear_vel * value,
                    self.radial_vel * value)
...
class RobotinhoController:
    ...
    def angular_vel(self, goal_pose, constant=6):
        ...
        if (pi / 2 - 0.1 < abs(steering_angle) < pi / 2 + 0.1 or pi - 0.1 <= abs(steering_angle) <= pi + 0.1) and abs(
                w) > 0.1:
            return w, True
        return w, False
