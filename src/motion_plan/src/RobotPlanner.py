...
class RobotinhoPlanner:
        ...
        while exploredPoint < len(self.explorationPoints):
            ...
            while not rospy.is_shutdown() and self.current_distance > self.distance_error and (
                ...
                print("Current Distance: ", self.current_distance, " Available Position: ", self.availablePosition,
                      "Current DesPosition ", msg.pose.pose.position)
    ...
    def goalBehavior(self):
        ...
        msg.pose.pose.position.y = meanPortPose.y + (ballEstimationPosition.y - meanPortPose.y) * \
                                   (ballEstimationPosition.x - meanPortPose.x) / (ballEstimationPosition.y - meanPortPose.x) #???????????????????????????????

        while sqrt(pow(msg.pose.pose.position.x - self.current_x, 2) + pow(msg.pose.pose.position.y - self.current_y,
